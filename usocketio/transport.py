...
class SocketIO:
    ...
    def _handle_packet(self, packet_type, data):
        if packet_type is None:
            ...
            LOGGER.warn("Unhandled packet %s: %s" % (packet_type, data))
    ...
    def _handle_message(self, message_type, data):
        if message_type == MESSAGE_EVENT:
            ...
            LOGGER.warn("Unhandled message %s: %s" % (message_type, data))
    ...
    def _handle_event(self, event, data=None):
        ...
        for handler in self._event_handlers.get(event, []):
            if __debug__:
                LOGGER.debug("Calling handler %s for event '%s'" % (handler, event))
            handler(data)
    ...
    def at_interval(self, approx_seconds):
        """Register an event handler to happen at an interval (s)."""

        def inner(func):
            if __debug__:
                LOGGER.debug("Registered %s to run at interval %s" % (func, approx_seconds))

            self._interval_handlers.append((approx_seconds, func))
